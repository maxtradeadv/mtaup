...
async function stock(t:string,a:string,b:string,p:Provider){
  ...
  const prices=p==='yahoo'?await yahooOhlcv(ticker,a,b):p==='idx'||p==='rapidapi'?await idxStock(ticker,a,b):await remoteCsvRange(a,b,[ticker]);
  let broker:any[]=[];
  if(p==='stockbit'){
