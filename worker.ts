...
import { brokerSummary as indexAlphaBrokerSummary } from './providers/indexalpha.ts';
import { remoteCsvRange, remoteCsvServerTimestamp } from './providers/remote-csv.ts';

type Provider='auto'|'yahoo'|'remote-csv';
...
const providerOf=(v:string|null):Provider=>['yahoo','remote-csv','auto'].includes(v||'')?v as Provider:(['yahoo','remote-csv'].includes(envGet('DATA_PROVIDER')||'')?envGet('DATA_PROVIDER') as Provider:'auto');
...
async function stock(t:string,a:string,b:string,p:Provider){
  ...
  const prices=p==='yahoo'?await yahooOhlcv(ticker,a,b):await remoteCsvRange(a,b,[ticker]);
  let broker:any[]=[];
  const iaKey=envGet('INDEX_ALPHA_API_KEY')?.trim();
  if(iaKey&&prices.length){
    const dates=[...new Set(prices.map(x=>String(x.date)).filter(Boolean))].slice(-20);
    for(let i=0;i<dates.length;i+=5){
      const batch=await Promise.allSettled(dates.slice(i,i+5).map(d=>indexAlphaBrokerSummary(ticker,d.replaceAll('-',''),iaKey)));
      for(const x of batch)if(x.status==='fulfilled')broker.push(...x.value);else console.warn('[BROKER]',String(x.reason));
    }
  }
...
async function sessionKey(){const access=envGet(ACCESS_CODE_ENV)?.trim();if(!access)return null;const secret=(envGet(SESSION_SECRET_ENV)?.trim()||access);return crypto.subtle.importKey('raw',enc.encode(secret),{name:'HMAC',hash:'SHA-256'},false,['sign','verify'])}
...
async function authGuard(req:Request){const configured=!!envGet(ACCESS_CODE_ENV)?.trim();if(!configured)return new Response('Private access is not configured. Set Cloudflare secret APP_ACCESS_CODE.',{status:503,headers:{'Content-Type':'text/plain; charset=utf-8','Cache-Control':'no-store'}});if(await validSession(req))return null;return loginPage()}
const files:any={'/':'index.html','/index.html':'index.html','/styles.css':'styles.css','/analysis.js':'analysis.js','/data-provider.js':'data-provider.js','/calibration.js':'calibration.js','/app.js':'app.js','/manifest.webmanifest':'manifest.webmanifest','/icon.svg':'icon.svg','/icon-180.png':'icon-180.png','/apple-touch-icon.png':'apple-touch-icon.png','/sw.js':'sw.js'};
...
export default {async fetch(req:Request,env:Env){runtimeEnv=env;const u=new URL(req.url);try{
  ...
  if(u.pathname==='/health'){const p=providerOf(null);return json({ok:true,provider:p,accessConfigured:!!envGet(ACCESS_CODE_ENV)?.trim(),sessionConfigured:!!envGet(SESSION_SECRET_ENV)?.trim(),sources:{remoteCsv:'Community daily CSV (IDX-derived via imq21)',yahoo:'Yahoo Finance (third-party historical)'}})}
  const guard=await authGuard(req);if(guard)return guard;
  if(files[u.pathname])return file(u.pathname,req);
  if(u.pathname==='/source-meta'){const requested=providerOf(u.searchParams.get('provider'));let sourceTimestamp='';if(requested==='remote-csv'||requested==='auto'){const now=new Date();const ds=now.toISOString().slice(0,10).replaceAll('-','');sourceTimestamp=await remoteCsvServerTimestamp(ds);if(!sourceTimestamp){const dt=new Date(`${ds.slice(0,4)}-${ds.slice(4,6)}-${ds.slice(6,8)}T00:00:00Z`);dt.setUTCDate(dt.getUTCDate()-1);sourceTimestamp=await remoteCsvServerTimestamp(dt.toISOString().slice(0,10).replaceAll('-',''))}}else if(requested==='yahoo'){try{const r=await fetch('https://query1.finance.yahoo.com/v8/finance/chart/%5EJKSE?range=5d&interval=1d',{headers:{Accept:'application/json','User-Agent':'Mozilla/5.0'}});if(r.ok){const j=await r.json() as any;const ts=j?.chart?.result?.[0]?.meta?.regularMarketTime;if(ts)sourceTimestamp=new Date(Number(ts)*1000).toISOString()}}catch(e){console.warn('[YAHOO META]',String(e))}}return json({ok:true,provider:requested,source:requested==='remote-csv'||requested==='auto'?'Community daily CSV (IDX-derived via imq21; not official IDX API)':'Yahoo Finance (third-party historical)',sourceTimestamp})}
