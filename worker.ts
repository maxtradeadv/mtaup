...
const providerOf=(v:string|null):Provider=>['yahoo','remote-csv','auto','stockbit','idx','rapidapi'].includes(v||'')?v as Provider:(['yahoo','remote-csv','stockbit','idx','rapidapi'].includes(envGet('DATA_PROVIDER')||'')?envGet('DATA_PROVIDER') as Provider:'auto');
...
async function stock(t:string,a:string,b:string,p:Provider){
  ...
  if(p==='stockbit'){
    ...
    const brokerFrom=new Date(new Date(b.slice(0,4)+'-'+b.slice(4,6)+'-'+b.slice(6,8)+'T00:00:00Z').getTime()-9*86400000).toISOString().slice(0,10);
    broker=await rapidBrokerSummary(ticker,brokerFrom,b.replace(/^(\d{4})(\d{2})(\d{2})$/,'$1-$2-$3'),envGet('RAPIDAPI_KEY'));
  }
