...
async function range(a:string,b:string,p:Provider,ts=ytickers,names?:Map<string,string>,diag?:any){
  ...
  if(p==='yahoo'){
    ...
    const failed:string[]=[];let success=0,empty=0;
    for(let i=0;i<jobs.length;i+=8){
      const batch=await Promise.allSettled(jobs.slice(i,i+8).map(x=>x.job));
      batch.forEach((x,j)=>{const ticker=jobs[i+j].ticker;if(x.status==='fulfilled'){if(x.value.length){d.push(...x.value);success++;}else{empty++;failed.push(ticker);}}else{failed.push(ticker);console.warn('[YAHOO]',ticker,String(x.reason));}});
    }
    if(diag)Object.assign(diag,{requested:ts.length,success,empty,failed:failed.length,failedTickers:failed.slice(0,20),rows:d.length});
  }else{d=await remoteCsvRange(a,b,ts);}
  cache.set(key,{at:Date.now(),data:d});return d;
}
