...
type Provider='auto'|'yahoo'|'remote-csv'|'stockbit';
interface Env { ASSETS: Fetcher; APP_ACCESS_CODE?: string; APP_SESSION_SECRET?: string; DATA_PROVIDER?: string; STOCKBIT_ACCESS_TOKEN?: string; }
let runtimeEnv: Env;
const envGet=(k:keyof Env)=>runtimeEnv?.[k] ?? '';
const providerOf=(v:string|null):Provider=>['yahoo','remote-csv','auto','stockbit'].includes(v||'')?v as Provider:(['yahoo','remote-csv','stockbit'].includes(envGet('DATA_PROVIDER')||'')?envGet('DATA_PROVIDER') as Provider:'auto');
